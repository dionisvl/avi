...
export function SearchBar() {
  ...
  const activeCity =
    cities?.find((c) => isCityObject(c) && c.id === selectedCityId) ||
    (cities?.find(isCityObject) as Exclude<City, string> | undefined);
  ...
  const handleSearch = () => {
    ...
    if (typeof activeCity?.id === "string" && activeCity.id.length > 0) {
      params.set("city_uuid", activeCity.id);
    }
  ...
  return (
    <div className="mt-5 md:mt-6">
      <div className="flex min-h-[66px] flex-col items-center gap-3 rounded-search border border-border bg-white/95 p-2 shadow-[0_18px_54px_rgba(42,38,80,0.08)] md:flex-row md:gap-0 md:p-0">
        <div className="flex min-h-[58px] flex-1 items-center gap-4 px-4 md:px-6">
          <Search className="size-5 shrink-0 text-[#7780a5]" />
          <Input
            type="text"
            ...
            className="h-auto border-0 bg-transparent p-0 text-[15px] font-medium text-text-primary placeholder:text-text-muted focus-visible:border-0 focus-visible:ring-0"
          />
        </div>
