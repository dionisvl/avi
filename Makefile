...
# DEPLOY to prod
deploy-api:
	ssh -p $(DEPLOY_PORT) $(DEPLOY_USER)@$(DEPLOY_HOST) "cd /home/avi && git pull && docker compose -p avi up -d --build --no-deps api"
...
deploy-fe: fe-admin-build
	...
	ssh -p $(DEPLOY_PORT) $(DEPLOY_USER)@$(DEPLOY_HOST) "cd /home/avi && docker compose -p avi up -d --build --no-deps fe-nextjs fe-admin"
